...
const jobData = [
  {
    puesto: 'Auxiliar de Reparto Logística',
      ...
      <table className="min-w-full table-auto">
        <thead>
          <tr className="bg-gray-200 text-left">
            ...
            <th className="px-4 py-2">Postulados</th>
        ...
        <tbody>
          {jobData.map((job, index) => (
            <tr key={index} className="border-b">
              <td className="px-4 py-2">{job.puesto}</td>
              <td className="px-4 py-2">{job.fecha}</td>
              <td className="px-4 py-2">{job.postulados}</td>
              <td className={`px-4 py-2 ${job.estado === 'Abierto' ? 'text-green-500' : 'text-red-500'}`}>
                {job.estado}
              </td>
            </tr>
          ))}
